...
class TransformerLM(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, vocab_size: int, context_length: int, 
                  num_layers: int, theta: float, weights: dict[str, torch.Tensor] | None = None):
        super().__init__()
        self.base_layer = TransformerBlock(d_model, num_heads, d_ff)
        self.embedding = Embedding(vocab_size, d_model)
        if weights is not None: self.embedding.load_state_dict({"weight": weights["token_embeddings.weight"]})
        self.linear = Linear(d_model, vocab_size)
        if weights is not None: self.linear.load_state_dict({"weight": weights["lm_head.weight"]})
        # self.linear.load_state_dict({"weights": weights["lm_head.weight"]})
        self.rms_norm_final = RMSNorm(d_model=d_model)
        if weights is not None: self.rms_norm_final.load_state_dict({"weight": weights["ln_final.weight"]})
